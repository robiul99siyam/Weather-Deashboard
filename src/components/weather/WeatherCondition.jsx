import TempMaxImage from "../../assets/icons/temp-max.svg";
import TempMinIamge from "../../assets/icons/temp-min.svg";
import HumadityImage from "../../assets/icons/humidity.svg";
import CloudyImage from "../../assets/icons/cloud.svg";
import WindImage from "../../assets/icons/wind.svg";
import { useContext } from "react";
import { weatherContext } from "../../context";
export default function WeatherCondition() {
  const { weather } = useContext(weatherContext);
  const { maxTemperature, minTemperature, humidity, wind, cloudPercentage } =
    weather;
  return (
    <div>
      <p className="text-sm lg:text-lg font-bold uppercase mb-8">
        the climate is <ul> {weather.climate}</ul>
      </p>
      <ul className="space-y-6 lg:space-y-6">
        <li className="text-sm lg:text-lg flex items-center justify-between space-x-4">
          <span>Temp max</span>
          <div className="inline-flex space-x-4">
            <p>{Math.round(maxTemperature)}°</p>
            <img src={TempMaxImage} alt="temp-max" />
          </div>
        </li>
        <li className="text-sm lg:text-lg flex items-center justify-between space-x-4">
          <span>Temp min</span>
          <div className="inline-flex space-x-4">
            <p>{Math.round(minTemperature)}°</p>
            <img src={TempMinIamge} alt="temp-min" />
          </div>
        </li>
        <li className="text-sm lg:text-lg flex items-center justify-between space-x-4">
          <span>Humadity</span>
          <div className="inline-flex space-x-4">
            <p>{humidity}%</p>
            <img src={HumadityImage} alt="humidity" />
          </div>
        </li>
        <li className="text-sm lg:text-lg flex items-center justify-between space-x-4">
          <span>Cloudy</span>
          <div className="inline-flex space-x-4">
            <p>{cloudPercentage}%</p>
            <img src={CloudyImage} alt="cloudy" />
          </div>
        </li>
        <li className="text-sm lg:text-lg flex items-center justify-between space-x-4">
          <span>Wind</span>
          <div className="inline-flex space-x-4">
            <p>{wind}km/h</p>
            <img src={WindImage} alt="wind" />
          </div>
        </li>
      </ul>
    </div>
  );
}
